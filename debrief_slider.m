...
for j = 1:length(ratingRectXpos) % ratingRectXpos accounts for all the rectangles, while scale_range would not
    ...
    if i==1
        scale_question = {'ho sentito che giocavo con:' ;'In il gioco di carte Patent Race,'}; % reverse order due to how y position is calculated away from center
        scale_instructions = {'Usa sempre i tasti freccia.'; 'Clicca SPAZIO per continuare.'};
        scale_middle_label = '?';
        ...
        scale_question = {'dipendente delle mie scelte:'; 'l''avversario cambiava la sua strategia'; 'In il gioco di carte Patent Race,'}; % reverse order due to how y position is calculated away from center 
        scale_instructions = {'Usa sempre i tasti freccia.'; 'Clicca SPAZIO per continuare.'};
        scale_hi_label = 'molto';
        scale_middle_label = 'neutrale';
        scale_low_label = 'per niente';
        dummy=imread('dummy.jpg'); %load image of human
