function [crtResponse, crtPrevious] = CRTquestionnaire(cfg, particNum, DateTime, window, windowRect, enabledKeys)
...
crtInputHelp = 'Se sbagli, premi "x" e "invio" e puoi riprovare.';
...
numIteration = height(crtprompt(:,1))-1;
numPrompts = width(crtprompt(1,:));
crtResponse = nan(numIteration, numPrompts);
...
for i = 1:numIteration;
    ...
    while aOK ~= 1
        ...
        thisChoice = str2num(GetEchoString(window, char(crtprompt.question(i)), 10, cfg.topTextYpos, cfg.textColor)); % displays string in PTB; allows backspace
...
while aOK ~= 1
    ...
    DrawFormattedText(window, crtInputHelp, 'center', cfg.waitTextYpos, cfg.p1Col);
    trialStartTime(numCrtPrevious) = GetSecs;
    thisChoice = GetEchoString(window, char(crtprompt.question(numCrtPrevious)), 10, cfg.topTextYpos, cfg.textColor); % displays string in PTB; allows backspace
    ...
    switch isempty(thisChoice)
        case 1 %deals with both cancel and X presses
            ...
            continue
        case 0
            crtPrevious = thisChoice;
            trialEndTime(numCrtPrevious) = GetSecs; % after choice
            aOK = 1;
    end
end

for i=1:numCrtPrevious
        trialLength(i) = trialEndTime(i)-trialStartTime(i);
end

Screen('Flip', window)

save(['sub' num2str(particNum) '-' num2str(DateTime) '_q4CRT'], 'crtResponse', 'crtPrevious', 'trialStartTime', 'trialEndTime', 'trialLength');
